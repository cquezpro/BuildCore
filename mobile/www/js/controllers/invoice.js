...
.controller('InvoiceCtrl', function($scope, $baseUrl, $stateParams, $http, $ionicActionSheet, $state, $ionicLoading, $ionicPopup, $window) {
  ...
  $scope.autoPay = function(invoice) {
    $state.transitionTo('app.vendorPaymentTerms', { vendorid: invoice.vendor_id, tab: 'autopay' });
  };
  ...
  $scope.updateItem = function(id, value, type, update_all) {
    ...
    if (update_all) {
      ...
      params.vendor_id = $scope.invoice.vendor.id;
      ...
      $http.put($baseUrl + '/api/v1/line_items/' + id, params).success(function(response) {
        console.log('success');
      }).error(function(response) {
        console.log('error', response);
      });
