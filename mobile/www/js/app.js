angular.module('billsync', ['ionic', 'Devise', 'billsync.controllers', 'billsync.filters', 'billsync.services','billsync.entities','billsync.directives','ui.mask', 'ngResource'])
...
.controller('AppCtrl', function($scope, $ionicModal, $timeout, $location, Auth, $ionicPopup, $window, $state, $rootScope, $http, $baseUrl,$ionicLoading, authorize, $compile) {
  $scope.loginData = {};
  $scope.newUser = {};
 ...
 $scope.submiting_request = false;
  ...
  $scope.submitForgotPasswordForm = function() {
    $scope.responseText = '';
    $http.post('/auth/password',{individual: $scope.individual}).success($scope.successForgotPassword).error($scope.errorForgotPassword);
  };
   ...
   $scope.submitPasswordChangeForm = function() {
    ...
    $http.put('/auth/password', {individual: $scope.individual}).success($scope.successPasswordChange).error($scope.errorPasswordChange);
  };
